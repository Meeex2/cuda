...
def swiglu_kernel(
    ...
    # Compute SwiGLU: y = Swish(x1) * x2
    swish_x1 = x1 * (1.0 / (1.0 + tl.exp(-x1)))  # Swish(x1) = x1 * sigmoid(x1)
    y = swish_x1 * x2
...
def swiglu_triton(x: torch.Tensor):
    ...
    swiglu_kernel[grid](x, y, n_elements, BLOCK_SIZE=1024)  # Launch kernel
    return y


# CPU Implementation of SwiGLU for Comparison
def swiglu_cpu(x: torch.Tensor):
    half_size = x.size(0) // 2
    x1 = x[:half_size]
    x2 = x[half_size:]
    swish_x1 = x1 * torch.sigmoid(x1)  # Swish(x1) = x1 * sigmoid(x1)
    return swish_x1 * x2  # SwiGLU(x) = Swish(x1) * x2
...
def test_swiglu():
    ...
    y_cpu = swiglu_cpu(x.cpu()).cuda()  # Move to GPU for comparison
