...
__global__ void elish_kernel(const float* input, float* output, int num_elements) {
    ...
    if (i < num_elements) {
        ...
        if (x >= 0.0f) {
            ...
            output[i] = (__expf(x) - 1.0f) * sigmoid;
        }
    }
}

void cpu_elish(const float* input, float* output, int num_elements) {
    for (int i = 0; i < num_elements; ++i) {
        ...
        if (x >= 0.0f) {
            ...
            output[i] = (std::exp(x) - 1.0f) * sigmoid;
        }
    }
}
...
int main() {
    ...
    cpu_elish(host_input.data(), cpu_output.data(), num_elements);
    ...
    elish_kernel<<<grid_size, block_size>>>(device_input, device_output, num_elements);
