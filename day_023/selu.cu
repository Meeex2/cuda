...
const float SELU_LAMBDA = 1.0507009873554804934193349852946f;
...
void cpu_selu(const float* input, float* output, int num_elements) {
    for (int i = 0; i < num_elements; ++i) {
        const float x = input[i];
        output[i] = (x > 0.0f) ? 
                   SELU_LAMBDA * x : 
                   SELU_LAMBDA * SELU_ALPHA * (std::exp(x) - 1.0f);
    }
}
...
int main() {
    ...
    cpu_selu(host_input.data(), cpu_output.data(), num_elements);
    ...
    selu_kernel<<<grid_size, block_size>>>(device_input, device_output, num_elements);
